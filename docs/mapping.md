...
| `engine_backend/approvals.py` | The durable approval ledger, reached as `store.approvals`: an operator's approval of one staged proposal (`record`), its single-use apply claim and target leases (`claim`, `finish_attempt`), stale-attempt recovery and the reconciliation claim (`recover_stale`, `claim_reconciliation`, `abort_reconciliation`, `finish_reconciliation`), and the append-only event history the portal renders. `EngineStore` still owns the connection and the OS-held `merchant_operation` lock; the ledger owns the rows |
